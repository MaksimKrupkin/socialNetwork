...
namespace api.Controllers
{
    ...
    public class ChatsController : ControllerBase
    {
        ...
        // Создать новый чат
[HttpPost]
public async Task<ActionResult<ChatDto>> Create(ChatDto chatDto)
{
    var chatModel = ChatMapper.ToChatModel(chatDto); // Преобразуем DTO в модель

    // Не устанавливайте chatModel.Id, он будет сгенерирован автоматически
    var createdChat = await _chatRepository.CreateAsync(chatModel);
    
    var createdChatDto = ChatMapper.ToChatDto(createdChat); // Преобразуем в DTO
