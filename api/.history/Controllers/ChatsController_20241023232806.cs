...
public async Task<ActionResult<ChatDto>> Create(CreateChatDto chatDto)
{
    ...
    // Обрабатываем создание чата с сообщениями
    foreach (var messageDto in chatDto.Messages)
    {
        var messageModel = MessageMapper.ToMessageModel(messageDto);
        messageModel.ChatId = chatModel.Id; // Set ChatId after creating the chat
        messageModel.SentAt = DateTime.UtcNow; // Устанавливаем дату отправки
        chatModel.Messages.Add(messageModel);  // Добавляем сообщение в чат
    }
