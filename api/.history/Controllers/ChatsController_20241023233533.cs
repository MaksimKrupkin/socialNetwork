...
public async Task<ActionResult<ChatDto>> Create(CreateChatDto chatDto)
{
    ...
    // Обрабатываем создание сообщений только после создания чата
    foreach (var messageDto in chatDto.Messages)
    {
        var messageModel = MessageMapper.ToMessageModel(messageDto);
        messageModel.ChatId = createdChat.Id; // Устанавливаем ChatId после создания чата
        messageModel.SentAt = DateTime.UtcNow; // Устанавливаем дату отправки
        await _messageRepository.CreateAsync(messageModel); // Предполагается, что у вас есть метод для сохранения сообщения
    }
